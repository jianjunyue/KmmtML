...
def repeatFeature():
    ...
    inpath='data/train30_v4.csv'
    outpath='data/train30_v5.csv'
	
	#此次有BUG，正确的写的应该是train.csv,但是没有来得及改。比赛模型一直用的test.csv
	#df_all=pd.read_csv('../data/origin/train.csv').drop(['label','conversionTime'],axis=1) 
	
    df_all=pd.read_csv('../data/origin/test.csv').drop(['label','conversionTime'],axis=1) 
...
def createTimeDict():
    ...
    df_train=pd.read_csv('data/train30_v1.csv').drop(['conversionTime','sort'],axis=1)
...
#反向统计间隔
def repeatFeatureRe():
    inpath='data/train30_v6.csv'
    outpath='data/train30_v7.csv'
    df_all=pd.read_csv('../data/origin/train.csv').drop(['label','conversionTime'],axis=1) 
    ...
    df_all2=pd.read_csv('data/train30_v1.csv')
    df_all2=pd.merge(df_all2,df_repeat,how='left',on='sort')
