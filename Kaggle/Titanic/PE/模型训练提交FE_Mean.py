...
# peName="_onehot_encoding"
peName="_mean_encoding"
titanicPath=FileUtil.dataKagglePath("titanic/")
path=titanicPath+"fe/train"+peName+".csv"
data=pd.read_csv(path)
path_test=titanicPath+"fe/test"+peName+".csv"
data_test=pd.read_csv(path_test)
y=data["Survived"]
train=data.drop(["PassengerId","Survived"],axis=1)

submission=pd.DataFrame()
submission["PassengerId"]=data_test["PassengerId"]
test=data_test.drop(["PassengerId"],axis=1)
...
submission.to_csv(titanicPath+"submission/XGBClassifier"+peName+".csv",index=None)
...
submission["Survived"]=y_pred
submission.to_csv(titanicPath+"submission/RandomForestClassifier"+peName+".csv",index=None)
# accuracy_score=metrics.accuracy_score(y_pred,y_test)
...
submission["Survived"]=y_pred
submission.to_csv(titanicPath+"submission/GradientBoostingClassifier"+peName+".csv",index=None)
# accuracy_score=metrics.accuracy_score(y_pred,y_test)
...
submission["Survived"]=y_pred
submission.to_csv(titanicPath+"submission/Stack"+peName+".csv",index=None)
# accuracy_score=metrics.accuracy_score(y_pred,y_test)
...
submission["Survived"]=y_pred
submission.to_csv(titanicPath+"submission/Blend"+peName+".csv",index=None)
# accuracy_score=metrics.accuracy_score(y_pred,y_test)
